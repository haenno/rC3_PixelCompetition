# Pacmac runs over a selected row on the screen... *wacka! wacka!
...
def wacka_wacka():
    ...
    zeile = input("Row? (0-1080, but better choose approx ~80-950) -> ")
    write_to_console("Letting Pacman lose...")
    threading.Thread(target=run_pacman, args=[int(zeile)]).start()
    write_to_console("  ..>> Eating row " + str(zeile))        
    sleep(2.5)
    while threading.active_count() != 1: 
        write_to_console(" > Waiting to eat it all up: "+ str(threading.active_count()-1))
        sleep(1.5)        
